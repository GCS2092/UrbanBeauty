...
async function buildReportPdf(data) {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(10).font('Helvetica').fillColor('#555555')
      .text(`Periode : ${formatDate(data.period.from)} au ${formatDate(data.period.to)}`, { align: 'right' });
    doc.text(`Genere le : ${formatDate(new Date())}`, { align: 'right' });
    ...
    // Resume financier
    sectionTitle(doc, 'Resume financier');
    row(doc, "Chiffre d'affaires (commandes livrees)", formatFcfa(data.financial.chiffreAffaires));
    row(doc, 'Cout des marchandises vendues (COGS)',   formatFcfa(data.financial.cogs));
    row(doc, 'Total depenses',                         formatFcfa(data.financial.totalExpenses));
    row(doc, 'Benefice estime',                        formatFcfa(data.financial.beneficeEstime), true);
    row(doc, 'Total facture',                          formatFcfa(data.financial.totalInvoiced));
    ...
    for (const p of data.products.topProducts) {
      row(doc, `  ${p.name} - ${p.quantity} unites`, formatFcfa(p.revenue));
    }

    // Factures
    sectionTitle(doc, 'Factures');
    row(doc, 'Total factures emises', String(data.invoices.total));
    for (const [status, count] of Object.entries(data.invoices.byStatus)) {
      row(doc, `  - ${status}`, String(count));
    }
    row(doc, 'Montant total facture', formatFcfa(data.invoices.totalInvoiced));
    ...
    row(doc, 'Produits epuises',              String(data.stock.outOfStock.length));
    ...
    if (data.stock.lowStock.length > 0) {
      ...
      for (const p of data.stock.lowStock) {
        row(doc, `  /!\\ ${p.name}`, `Stock : ${p.stock} (alerte : ${p.alert})`);
      }
    }

    if (data.stock.outOfStock.length > 0) {
      doc.moveDown(0.5);
      doc.font('Helvetica-Bold').fontSize(10).text('Produits epuises :');
      doc.moveDown(0.3);
      for (const p of data.stock.outOfStock) {
        doc.font('Helvetica').fontSize(10).text(`  [epuise] ${p.name}`, { indent: 10 });
        doc.moveDown(0.3);
      }
    }

    // Depenses
    sectionTitle(doc, 'Depenses');
    row(doc, 'Total depenses', formatFcfa(data.expenses.total), true);
    ...
    doc.text(
      `Document genere automatiquement - ${companyName}`,
      50,
      doc.page.height - 50,
