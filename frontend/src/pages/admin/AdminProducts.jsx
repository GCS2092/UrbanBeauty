...
function ImageUploader({ images, onChange, token, variantColors = [] }) {
  ...
  const setColor = (idx, color) => {
    onChange(
      images.map((img, i) =>
        i === idx ? { ...img, color: color || null } : img,
      ),
    );
  };
          ...
          {uploading ? (
            ...
            <div className="flex flex-col items-center gap-1.5">
              ...
              <p className="text-xs text-gray-400">
                JPEG · PNG · WebP · AVIF — max 5 Mo par image
              </p>
            </div>
          )}
        </div>
          ...
          {images.map((img, idx) => (
            ...
            >
              ...
              <div className="relative shrink-0">
                ...
                />
                {img.isMain && (
                  <span className="absolute top-0.5 left-0.5 bg-black text-white text-[9px] font-bold px-1 py-0.5 rounded-full">
                    ★
                  </span>
                )}
              </div>
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Chaque combinaison taille + couleur a son propre stock
          </p>
        </div>
        <button
          type="button"
      ...
      {variants.length === 0 && (
        <div className="border-2 border-dashed border-gray-200 rounded-xl p-4 text-center text-gray-400 text-sm">
          Aucune variante — le produit a un stock unique.
          <br />
          <span className="text-xs">
            Cliquez "+ Ajouter" pour créer des variantes taille/couleur.
          </span>
        </div>
      )}
      ...
      {variants.length > 0 && (
        <div className="space-y-2">
          ...
          {variants.map((v, idx) => (
            <div
              key={idx}
              className="grid grid-cols-[1fr_1fr_80px_32px] gap-2 items-start"
            >
                ...
                <div className="flex gap-1 flex-wrap">
                  {colorSuggestions.map((c) => (
                    <button
                      key={c}
                      type="button"
                      onClick={() => updateVariant(idx, "color", c)}
                      className={`text-[10px] px-1.5 py-0.5 rounded border transition-colors ${
                        v.color === c
                          ? "bg-gray-900 text-white border-gray-900"
                          : "border-gray-200 text-gray-500 hover:border-gray-400"
              ...
              <button
                ...
                className="w-8 h-8 mt-0.5 flex items-center justify-center rounded-lg border border-red-200 text-red-400 hover:bg-red-50 transition-colors text-xs font-bold"
              >
                ✕
              </button>
            </div>
...
export default function AdminProducts() {
  ...
  const validate = () => {
    ...
    if (form.purchasePrice !== "" && Number(form.purchasePrice) < 0)
      errors.purchasePrice = "Prix achat invalide";
    form.variants.forEach((v, i) => {
      if (!v.size.trim() && !v.color.trim())
        errors[`variant_${i}`] =
          `Variante ${i + 1} : taille ou couleur requise`;
      if (v.stock === "" || Number(v.stock) < 0)
        errors[`variant_stock_${i}`] = `Variante ${i + 1} : stock invalide`;
    });
    return errors;
  };
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        {(() => {
                          ...
                          return (
                            <span
                              className={`font-semibold ${available <= 5 ? "text-red-600" : "text-emerald-700"}`}
                            >
                              {available}
                            </span>
                          );
            ...
            >
              ...
              <div className="rounded-xl border border-amber-100 bg-amber-50 p-3 space-y-1">
                <label className="block text-sm font-medium text-amber-800 mb-1">
                  ...
                  <span className="text-amber-500 text-xs font-normal">
                    (optionnel — utilisé pour les marges)
                  </span>
                </label>
                <input
                  type="number"
                  ...
                  Number(form.purchasePrice) > 0 && (
                    <p className="text-xs text-amber-700 mt-1">
                      ...
                      <span className="font-semibold">
                        {formatPrice(
                          Number(form.price) - Number(form.purchasePrice),
                        )}
                      </span>{" "}
                      (
                      {Math.round(
              ...
              <div className="rounded-xl border border-blue-100 bg-blue-50 p-3">
                <label className="block text-sm font-medium text-blue-800 mb-1">
                  ...
                  <span className="text-blue-400 text-xs font-normal ml-1">
                    (laisser vide = visible sur toutes les boutiques)
                  </span>
                </label>
                <select
                  name="storeId"
              ...
              {form.variants.length > 0 && (
                <div className="border border-gray-100 rounded-xl p-3 space-y-2 bg-gray-50">
                  ...
                  <p className="text-xs text-gray-400">
                    Définit ce que le client choisit en premier côté boutique
                  </p>
                  <div className="flex gap-2">
                    <button
                      type="button"
                      ...
                      className={`flex-1 py-2 px-3 rounded-lg border text-sm font-medium transition-colors ${
                        (form.variantDisplayMode || "SIZE_FIRST") ===
                        "SIZE_FIRST"
                          ? "bg-gray-900 text-white border-gray-900"
                    ...
                    <button
                      ...
                      onClick={() =>
                        setForm((p) => ({
                          ...p,
                          variantDisplayMode: "COLOR_FIRST",
                        }))
                      }
                      className={`flex-1 py-2 px-3 rounded-lg border text-sm font-medium transition-colors ${
